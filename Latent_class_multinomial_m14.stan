...
parameters {
  ...
  array[3] real sex_coef_sens; // Coefficients for days_since_onset on sensitivity for each test
...
transformed parameters {
  ...
  // Compute sensitivities and specificities for each individual for each test
  for (i in 1:N) {
    for (j in 1:3) {
       logit_sens[i, j] = (intercept_sens[j] + age_coef_sens[j] * age_bin[i,1] + sex_coef_sens[j] * sex[i,1]); 
...
model {
 // Define the priors for intercept and coefficients 
  ...
  sex_coef_sens[1] ~ normal(0,1);
  sex_coef_sens[2] ~ normal(0,1);
  sex_coef_sens[3] ~ normal(0,1);
