...
transformed parameters {
  ...
  for (i in 1:N) {
    for (j in 1:3) {
       logit_sens[i, j] = (intercept_sens[j] + onset_coef_sens[j] * days_since_onset[i,1]); 
...
model {
 // Define the priors for intercept and coefficients 
  ...
  // intercept_sens[1] ~ normal(1.99,0.5); // PCR
  // intercept_sens[2] ~ normal(2.59,0.5); // RDT
  // intercept_sens[3] ~ normal(1.99,0.5); // ELISA
  // 
  // logit_spec[1] ~ normal(4.59,0.2); // PCR
  // logit_spec[2] ~ normal(4.59,0.2); // RDT
  // logit_spec[3] ~ normal(4.59,0.2); // ELISA
