...
parameters {
  ...
  matrix <upper=0> [3, 3] spline_coef_sens;       // Coefficients for spline terms on sensitivity for each test
  array[3] real<lower=0> logit_spec; //removing the lower bound of 0 results in model getting stuck in local minimum
}
...
transformed parameters {
  ...
  for (i in 1:N) {
    for (j in 1:3) {
        logit_sens[i, j] = intercept_sens[j] + dot_product(spline_basis[i], spline_coef_sens[j]);
  ...
  // Probability of being in each profile doesn't change based on test results
...
// PCR
spline_coef_sens[1,1] ~ normal(0,0.25); // steepest decline in first chunk of time, almost to 0 by day 20
spline_coef_sens[1,2] ~ normal(0,0.001); // flat at 0 sens
spline_coef_sens[1,3] ~ normal(0,0.001); // flat at 0 sens

// RDT
spline_coef_sens[2,1] ~ normal(0,0.001); // very small decline in first chunk of time. 0.000001
spline_coef_sens[2,2] ~ normal(0,0.0001); 
spline_coef_sens[2,3] ~ normal(0,0.000001); // flat at 0 sens

// ELISA
spline_coef_sens[3,1] ~ normal(0,0.001); // very small decline in first chunk of time. 0.000001
spline_coef_sens[3,2] ~ normal(0,0.0001); 
spline_coef_sens[3,3] ~ normal(0,0.000001); // flat at 0 sens
