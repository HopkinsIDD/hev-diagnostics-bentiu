...
real phi_base; // Intercept for estimating time varying logit_phi. Same for everyone, give it a prior
  ...
  real coef_month9;
  real coef_month10;
  array[2] real intercept_sens; //Intercept for sensitivity regression ONLY FOR PCR
  matrix <upper=0> [2,3] spline_coef_sens;      
  array[3] real <lower=0> logit_spec; 
  real logit_sens_pcr;  //needs its own param bc/s no longer adjusting for anything
}
...
transformed parameters {
  ...
  for (i in 1:N) {
    // PCR does not adjust for days since jaundice onset
    logit_sens[i,1] = logit_sens_pcr;
    sens_all[i, 1] = inv_logit(logit_sens[i, 1]);
    
    // RDT uses cubic spline
    logit_sens[i, 2] = intercept_sens[1] + dot_product(spline_basis[i], spline_coef_sens[1,]);
    sens_all[i, 2] = inv_logit(logit_sens[i, 2]);

    // ELISA uses cubic spline
    logit_sens[i, 3] = intercept_sens[2] + dot_product(spline_basis[i], spline_coef_sens[2,]);
...
model {
 // Define the priors for intercept and coefficients 
  logit_sens_pcr ~ normal(1.5,0.5); // PCR 
  intercept_sens[1] ~ normal(2.2,0.5); // RDT
  intercept_sens[2] ~ normal(2.3,0.5); // ELISA
...
// RDT
spline_coef_sens[1,1] ~ normal(0,0.001); 
spline_coef_sens[1,2] ~ normal(0,0.0001); 
spline_coef_sens[1,3] ~ normal(0,0.000001); 

// ELISA
spline_coef_sens[2,1] ~ normal(0,0.001); 
spline_coef_sens[2,2] ~ normal(0,0.0001); 
spline_coef_sens[2,3] ~ normal(0,0.000001); 
