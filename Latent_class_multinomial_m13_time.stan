...
parameters {
  ...
  real coef_month9;
  real coef_month10;
  array[1] real intercept_sens; //Intercept for sensitivity regression for each test. 
  array[1] real <upper=0> onset_coef_sens; // Coefficients for days_since_onset on sensitivity for each test
  array[3] real <lower=0> logit_spec; 
  real logit_sens_rdt;  //needs its own param bc/s no longer adjusting for anything
...
transformed parameters {
  ...
  for (i in 1:N) {
    // PCR continuous
    logit_sens[i,1] = intercept_sens[1] + onset_coef_sens[1] * days_since_onset[i,1];
    sens_all[i, 1] = inv_logit(logit_sens[i, 1]);
    
    // RDT does not adjust for days since onset
    logit_sens[i, 2] = logit_sens_rdt;
    sens_all[i, 2] = inv_logit(logit_sens[i, 2]);

    // ELISA does not adjust for days since onset
...
model {
 // Define the priors for intercept and coefficients 
  ...
  logit_sens_rdt ~ normal(2.2,0.5); // RDT
