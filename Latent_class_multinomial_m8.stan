...
parameters {
  ...
  array[3] real sex_coef_sens; // Coefficients for sex on sensitivity for each test
...
transformed parameters {
  ...
  for (i in 1:N) {
    for (j in 1:3) {
       logit_sens[i, j] = (intercept_sens[j] + onset_coef_sens[j] * days_since_onset[i,1] + sex_coef_sens[j] * sex[i,1]); 
...
model {
 ...
 sex_coef_sens[1] ~ normal(0,2);
 sex_coef_sens[2] ~ normal(0,2);
 sex_coef_sens[3] ~ normal(0,2);
