...
parameters {
  ...
  real coef_month9;
  real coef_month10;
  array[2] real intercept_sens; //Intercept for sensitivity regression for each test. 
  matrix <upper=0> [2,3] spline_coef_sens;      
  array[3] real <lower=0> logit_spec; 
  real logit_sens_elisa; //needs its own param bc/s no longer adjusting for anything
...
transformed parameters {
  ...
  for (i in 1:N) {
    // PCR spline basis
    logit_sens[i, 1] = intercept_sens[1] + dot_product(spline_basis[i], spline_coef_sens[1,]);
    sens_all[i, 1] = inv_logit(logit_sens[i, 1]);

    // RDT spline basis
    logit_sens[i, 2] = intercept_sens[2] + dot_product(spline_basis[i], spline_coef_sens[2,]);
    sens_all[i, 2] = inv_logit(logit_sens[i, 2]);

    //  ELISA does not adjust for days since jaundice onset
    logit_sens[i,3] = logit_sens_elisa;
...
model {
 // Define the priors for intercept and coefficients 
  ...
  logit_sens_elisa ~ normal(2.3,0.5); // ELISA
...
// PCR
spline_coef_sens[1,1] ~ normal(0,0.25); 
spline_coef_sens[1,2] ~ normal(0,0.001); 
spline_coef_sens[1,3] ~ normal(0,0.001); 
