...
transformed parameters {
  ...
  for (i in 1:N) {
    // PCR uses days_since_onset
    logit_sens[i, 1] = intercept_sens[1] + onset_coef_sens[1] * days_since_onset[i, 1];
    sens_all[i, 1] = inv_logit(logit_sens[i, 1]);

    // RDT uses days_since_onset
    logit_sens[i, 2] = intercept_sens[2] + onset_coef_sens[2] * days_since_onset[i, 1];
    sens_all[i, 2] = inv_logit(logit_sens[i, 2]);

    // ELISA uses spline
    logit_sens[i, 3] = intercept_sens[3] + dot_product(spline_basis[i], spline_coef_sens);
...
// Only for PCR and RDT
 onset_coef_sens[1] ~ normal(0,0.05); //PCR
 onset_coef_sens[2] ~ normal(0,0.01); //RDT
 
 // Priors for spline coefficients for ELISA
spline_coef_sens[1] ~ normal(0,0.001); 
spline_coef_sens[2] ~ normal(0,0.0001); 
spline_coef_sens[3] ~ normal(0,0.000001); 
